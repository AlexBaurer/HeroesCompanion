...
Дополнение (code review, пункт 4): `decodeAll` теперь читает записи с толерантностью v1 — добавлен `decodeTolerant`/`decodeMapTolerant`: отсутствующие/нулевые поля игрока подставляются (`''`/`0`, как `as String? ?? ''` в v1), пропускаются только записи, нечитаемые даже v1 (битый JSON, неверные типы, 0 или >4 игроков). Строгий `decode` сохранён для round-trip. Тесты дополнены кейсами толерантности.
